...
import argparse
from our_model_step import OurDDIMScheduler
import os
import tqdm
...
def main():
    ...
    if not os.path.exists('./results/ddim'):
        os.makedirs('./results/ddim', exist_ok=True)

    args.prefix = f'./results/ddim/{args.prefix}'

    fix_seed(args.seed)
    print("seed fixed to ",args.seed)

    scheduler = OurDDIMScheduler.from_pretrained("google/ddpm-cifar10-32")
    ...
    classifier=torch.nn.DataParallel(clsf).to('cuda')
    classifier.eval()
    ...
    def cond_fn(x, t, y=None, classifier=None, orig_inputs=None):
        ...
        for new_sample_t in new_scheduler.timesteps:
            if new_sample_t >= t or steps >= 5:
                ...
                steps+=1
            x_list[-1] = x_list[-1].detach().requires_grad_(True)
        ...
        grads = torch.autograd.grad(selected.sum(), x_list[-1])[0]# * 10.0
        ...
        return clipped_grads * scheduling_weight, x_list[-1]
